...
# 颤音化
def vibrato(audio_frames, fluct, n, sr=22050, frame_length=2048, hop_length=512):
    """参数
        audio_frames: 要调整的音频帧
        fluct: 每帧的波动值
        n: 帧数量
        sr: 音频的采样率
        frame_length: 帧长度
...
def vibrato_sine(y, A=1, w=1, b=0, T=10, sr=22050, frame_length=2048, hop_length=512):
    ...
    res_audio = vibrato(audio_frames, fluct, n, sr, frame_length, hop_length)
...
# 频率逐渐增高的正弦波颤音
def vibrato_increase_f_sine(y, A=1, p=1, sr=22050, frame_length=2048, hop_length=512):
    """参数
        y: 待颤音化的音频信号
        A: 振幅
        p: 频率变化程度 1-10
...
# 频率逐渐降低的正弦波颤音
def vibrato_decrease_f_sine(y, A=1, p=1, sr=22050, frame_length=2048, hop_length=512):
    """参数
        y: 待颤音化的音频信号
        p: 频率变化程度 1-10
        sr: 采样率
...
############################ 以下是提供给 gradio 的 API ############################
# 标准正弦波颤音 API
def vibrato_sine_api(filename, A=1, w=1, b=0, T=8, sr=44100, use_gradio=True):
    """参数
        filename: 文件名, 默认接收自 gradio
        A, w, b: 三角函数 A * sin(wx) + b
        T: 振动的周期数
        sr: 采样率
        use_gradio: 是否接收自 gradio
        ---------------------------------------
    ...
    result_audio_path = 'gradio_temp/vibrato_result_audio.wav'
    sf.write(result_audio_path, result_audio_data, sr)

    origin_fundfreq_path = 'gradio_temp/vibrato_origin_fundfreq.png'
    save_fundfreq_figure(audio_data, save_filename=origin_fundfreq_path)
    result_fundfreq_path = 'gradio_temp/vibrato_result_fundfreq.png'
...
# 频率均匀分布的正弦波颤音
def vibrato_uniformsine_api(filename, A=1, low=100, high=300, sr=44100, use_gradio=True):
    """参数
        filename: 文件名, 默认接收自 gradio
        A: 振幅
        low: 频率下限
...
# 频率正态分布的正弦波颤音
def vibrato_gausesine_api(filename, A=1, mean=0, square=1, sr=44100, use_gradio=True):
    """参数
        filename: 文件名, 默认接收自 gradio
        A: 振幅
        mean: 频率均值
    ...
    result_audio_data = vibrato_uniformsine(audio_data, A=A, mean=mean, square=square, sr=sr)
...
# 频率逐渐降低的正弦波颤音
def vibrato_decrease_f_sine_api(filename, A=1, p=1, sr=44100, use_gradio=True):
    """参数
        filename: 文件名, 默认接收自 gradio
        A: 振幅
        p: 频率的改变速率
    ...
    result_audio_data = vibrato_decrease_f_sine(audio_data, A=A, p=p, sr=sr)
...
# 频率逐渐升高的正弦波颤音
def vibrato_increase_f_sine_api(filename, A=1, p=1, sr=44100, use_gradio=True):
    """参数
        filename: 文件名, 默认接收自 gradio
        A: 振幅
        p: 频率的改变速率
    ...
    result_audio_data = vibrato_increase_f_sine(audio_data, A=A, p=p, sr=sr)
